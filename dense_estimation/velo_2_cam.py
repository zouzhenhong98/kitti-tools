...
def add_pc_to_img(img_path, coor, saveto=None):
    if img_path==None:
        ...
        img = np.zeros((352,1216),dtype=np.uint8)
    else:
    ...
    #tmp = np.clip(tmp, 0, 85)
    tmp = tmp * 256
    #tmp = tmp.astype(np.uint16)
    print(tmp.dtype)
    
    #cv2.imwrite('bit16.png', tmp)

    if saveto==None:
        cv2.imshow('compose',img)
    else:
        cv2.imwrite(saveto,tmp)
...
def lidar_to_camera_project(trans_mat, 
    ...
    value = data[2:,:] # height, reflectance, dist2d, dist3d
    ...
    x_range = np.where(((pixel[0]>=0) & (pixel[0]<pixel_range[0])))  # 对于训练集，会超出1216
...
if __name__ == "__main__":
    ...
    # add pixels to image
    temp, temp1 = add_pc_to_img(img_path=image_path, coor=pixel, saveto='./result/'+filename+'_composition.png')
